...
const addRing = (ringImg) => {
    const textLoader = new THREE.TextureLoader();
    const ringGeometry = new THREE.TorusGeometry(13, 3, 3, 100);
...
const addStar = (data) => {
    ...
    if (data.img === 'img/saturn_bg.jpg') {
        // star.add(addRing(star, 'img/saturn_bg.jpg'));
    }
...
function animate() {
    ...
    orbits.forEach((orbit, index) => {
        if(index) {
            orbit.rotation.z += star_data[index].velocity * 0.000001;
        }
        stars[index].rotation.y += star_data[index].rotVelocity * 0.000365;
    })
    // 渲染
